...
# Function to log emotion data to database
...
from emotion_user_features import UserFeatureExtractor
from emotion_recommendation_nn import EmotionVideoNN
from data_generator import TrainingDataGenerator
from integration_module import NeuralRecommender

# Initialize NN components
neural_feature_extractor = UserFeatureExtractor()
neural_emotion_nn = EmotionVideoNN()
neural_data_generator = TrainingDataGenerator(emotion_video_mapping=EMOTION_VIDEO_MAPPING)
neural_recommender = NeuralRecommender(
    emotion_nn=neural_emotion_nn,
    feature_extractor=neural_feature_extractor,
    data_generator=neural_data_generator,
    youtube_recommender=st.session_state.youtube_recommender
)

# Train or load model
neural_recommender.train_model(force_retrain=False)
        ...
        with col2:
            ...
            if st.button("Train Neural Model"):
                with st.spinner("Training neural network..."):
                    success = neural_recommender.train_model(force_retrain=True)
                    if success:
